...
//Returns: integer – the index position of the inserted reminder
//
//Behavior: 
...
size_t Calendar::addRem(const Reminder &r)	
{
	if (numRem == 0) {	//insert reminder into empty array
	...
	else if (numRem != 0 && numRem < MAX_REM) {	//partially filled calendar
		if (r <= remArr[0]) {	//reminder to be inserted is the most earliest
		...
		else {		// reminder to be inserted is either in the middle or at the end of the partially filled calendar
			size_t index = 0;
			while (index < numRem && r > remArr[index]) {
				index++;
			}
			if (index == numRem) {	//reminder to be inserted is the most recent
				remArr[index] = r;
				numRem++;
				return index;
			}
			else {	//reminder is to be inserted at the index "index" of the partially filled array
				size_t shifter = numRem;
				while (shifter > index) {
					remArr[shifter] = remArr[shifter - 1];
					shifter--;
...
string Calendar::displayRem(size_t index) const	
{
	...
	else if (numRem != 0 && index >= 0 && index < numRem) {		//non-empty calendar with valid index
...
{
	int index = -1;
	for (int i = numRem - 1; i >= 0; i--) {	//go from the last element to the first element in order to avoid having to use a break statement
		if (remArr[i].getDate() == d) {
			index = i;
		}
	}
...
size_t Calendar::deleteRem()
{
	...
	if (today <= remArr[0].getDate()) {	//there are no reminders earlier than today's date
		return 0;
	}
...
size_t Calendar::deleteRem(size_t index)	
{
	if (index < 0 || index >= numRem) {		//invalid index
		...
		if (index == numRem - 1) {	//delete the latest reminder, so nothing to shift
			numRem = numRem - 1;
			return 1;
		}
