...
void main(void)
{
	...
	try {
		Reminder test = cal1.getRem(cal1.getNumRem());	//test getRem(size_t index), attempt to get the reminder at an invalid index
		cout << "\nTHIS MESSAGE SHOULD NOT BE SEEN. An overflow exception was not thrown!!" << endl;
	}
	catch (std::invalid_argument &excpt)
	{
	...
	if (cal1.displayRem(cal1.getNumRem()) != "") {	//test displayRem(size_t index), attempt to display a string of the reminder at a invalid index
		cout << "displayRem failed to recognize the invalid index" << endl;
		cout << "displayRem returned the following reminder: " << cal1.displayRem(cal1.getNumRem()) << endl;
	}

	if (cal1.displayRem(-1) != "") {
		cout << "displayRem failed to recognize the invalid index" << endl;
		cout << "displayRem returned the following reminder: " << endl;
	}
	...
	if (cal1.deleteRem(cal1.getNumRem()) != 0) {		//tests whether deleteRem returns 0 when given an invalid index
